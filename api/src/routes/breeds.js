...
app.post("/newTemperament", async (req, res) => {
  try {
    let {
      name
    } = req.body;
    const newTemperament = await Temperament.create({name: name});
    res.status(200).json({message: "El nuevo temperamento: " + newTemperament.name + " se creo exitosamente"});
  }catch (error) {
